...
trace to the 1-ply piloting agent, and a learned evaluator with an
identity-free card encoding partially closes them. Simulated meta
...
\label{sec:shocks}
Table~\ref{tab:buzzard_series} and Fig.~\ref{fig:dissociation} show
the monthly series around the September nerf. At the card level,
...
\begin{axis}[
  ...
  legend style={font=\scriptsize, at={(0.97,0.97)}, anchor=north east,
                draw=vizaxis, fill=white, fill opacity=0.9, text opacity=1},
...
\addlegendentry{Buzzard adoption within Hunter (real)}
\addplot[vizorange, line width=1.2pt, dash pattern=on 4pt off 2.2pt]
  coordinates {(6,73.4) (8.72,73.4) (8.72,31.8) (12,31.8)};
\addlegendentry{model: seed input, then predicted 31.8\%}
\addplot[vizgray, line width=1.1pt, dashed,
         mark=square*, mark size=1.5pt, mark options={solid, fill=vizgray}]
  coordinates
  {(6,31.2) (7,30.4) (8,27.5) (9,32.6) (10,35.0) (11,34.7) (12,29.4)};
\addlegendentry{Hunter share of all decks (context)}
\draw[vizgray, dotted, line width=1pt] (axis cs:8.72,0) -- (axis cs:8.72,85)
  node[pos=0.13, left, font=\scriptsize, color=vizink] {nerf (Sept.~22)};
\end{axis}
\end{tikzpicture}
\caption{The card--class dissociation, with the model's prediction.
The nerf collapses Buzzard's within-Hunter adoption (blue). The
orange step shows the model: its flat pre-nerf segment is the input
it was given (73.4\% adoption in the real seed decks), and the drop
to 31.8\% is its prediction of the post-nerf level. Hunter's class

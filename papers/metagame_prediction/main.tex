...
$-0.075$ to $+0.34$; Mage rises to $+0.60$), which points to
mechanism coverage of the training data, and not model capacity, as
the binding constraint. Given only the patch text, the net assigns
the correct sign to both 2014 nerfs, agreeing with the linear agent
on Buzzard and improving on Leeroy. The two sequencing misses,
Death's Bite and Duplicate, persist under every static evaluator we
tested; no change of evaluation removes a limitation that lives in
the piloting policy.

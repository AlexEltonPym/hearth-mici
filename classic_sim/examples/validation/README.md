...
**But closing the loop fails.** Retraining the net on self-play games
generated BY beam search (--selfplay-agent beam, an AlphaZero-style
deviation from DouZero's search-free training) made things dramatically
worse: gen0 0.450, gen1 0.283 (p=5e-26) vs the 0.550 champion — a
collapse, not a plateau, and worse than greedy-generated data ever was.
Interpretation: whole-plan commitment produces far less trajectory
diversity than greedy's per-action re-deciding, starving Monte Carlo value
targets of exploration. Search helps at play time and hurts as a data
generator; DouZero's "no search in the training loop" is load-bearing, not
convention.

Files: strategy.BeamSearch, tests/beam_search_tests.py (9 tests incl. the
stale-plan regression), beam_ladder.py, --selfplay-agent in
train_value_net.py. Data: value_net_naxx/beam_ladder.json,
value_net_naxx_beam_selfplay/training_log.csv.

...
def evaluate_fitness(agent, sample):
  ...
  sample_elite = agent.parent_archive.vals_to_entry(sample_x, sample_y, missing_behaviour="closest")
  return sample_elite['fitness']*-1

def meta_evaluation():
  agent_profile = [1,1,1,1,1,1]
  meta_archive = MetaArchive(x_title="Hand size", y_title="Turns", x_range=(1, 9), y_range=(9, 35), num_buckets=40, archive_names=["mage", "hunter", "warrior"], num_agents=100, agent_profile=agent_profile)
  meta_archive.load("metagame/mage.json", "metagame/hunter.json", "metagame/warrior.json")
  


  meta_archives = run_agent_training(meta_archive)

  render_animation(meta_archives)

def train_agents_one_step(meta_archive):
  all_agents_finished = 0
  for agent in meta_archive.agents:
    if(not agent.es.stop()):
      ...
      all_agents_finished+=1
  return all_agents_finished == len(meta_archive.agents)
  

def run_agent_training(meta_archive):
  meta_archives = []
  done_training = False
...
def render_animation(meta_archives):
  ...
  ani = FuncAnimation(fig, animate, init_func=init_animation, frames=len(meta_archives), interval=20, repeat=repeating_animation, fargs=[meta_archives, fig, axis, repeating_animation])
  plt.show()

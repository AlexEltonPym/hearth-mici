...
class Game():
  ...
  def get_playable_spells_actions(self, player):
    ...
    for card in filter(lambda card: card.card_details['mana'] <= player.current_mana and card.card_details['card_type'] == 'spell', player.hand):
  ...
  def get_available_actions(self, player):
    ...
    available_actions.append({'action_type': Actions.END_TURN, 'source': player, 'target': 'board'})
    return available_actions


  def deck_to_hand(self, player):
    new_card = player.deck.deck.pop()
  ...
  def deck_to_graveyard(self, player):
    ...
    player.graveyard.append(new_card)
  ...
  def get_available_effect_targets(self, player, card):
    ...
    for effect in card.card_details['effects']:
      ...
      if filters == Filters.ENEMY or filters == Filters.ALL:
        for card in player.other_player.board:
          if targets == Targets.MINIONS or targets == Targets.MINIONS_OR_HEROES or card.card_details['minion_type'] == targets:
            available_targets.append(card)
        if targets == Targets.HEROES or targets == Targets.MINIONS_OR_HEROES:
  ...
  def get_hero_attack_actions(self, player):
    ...
    if player.attack > 0 and not player.has_attacked:
      for target in player.game.get_available_targets(self):
  ...
  def get_playable_minion_actions(self, player):
    ...
    if len(player.board) < 7:
      for card in filter(lambda card: card.card_details['mana'] <= player.current_mana and card.card_details['card_type'] == 'minion', player.hand):
        has_battlecry = False
        for effect in card.card_details['effects']:
          if effect['trigger'] == Triggers.BATTLECRY:
            has_battlecry = True
